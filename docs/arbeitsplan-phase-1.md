...
**Notizen:** DOI-Dublette kann technisch erst beim Verarbeiten (Paket-4-Job) erkannt werden, nicht beim reinen Datei-Drop – der Unique-Index auf `sources.doi` verhindert ohnehin, dass zwei Zeilen dieselbe DOI tragen; `doi.py` prüft jetzt vor dem Schreiben per SELECT und markiert die neuere Quelle als `needs_review` mit Verweis auf die Originalquelle, statt an der DB-Constraint abzustürzen. Titel-Dubletten (kein DB-Constraint möglich) über neuen Job `detect-duplicates`: paarweiser `difflib`-Abgleich alle-gegen-alle (Schwelle 0.85, bei ~150 Quellen trivial schnell), ältere Quelle bleibt Referenz, neuere wird markiert. Bestand eingespielt: 141 echte PDFs aus dem überarbeiteten `Testordner` (BITA-DT, DT-Sachversicherung, Einleitung, Sachversicherung-BITA, neu, inkl. aller „_Ergaenzend (nicht in Citavi)"-Unterordner; „Nicht_verwenden" bewusst ausgeschlossen, Entscheidung im Chat) plus die 14 aus früheren Paketen = 155 Quellen gesamt. Ergebnis der vollen Pipeline: 56 DOIs gefunden, 67 Quellen `complete`, 88 `needs_review` (59 ohne jeden Treffer, 18 mit zu unsicherer Titelsuche, 4 mit unvollständigen Pflichtfeldern, 7 echte DOI-Dubletten), 36 Rankings gefunden, 11 zusätzliche Titel-Dubletten-Hinweise, 0 Quellen bleiben in `processing`/`failed` hängen. Reale Dubletten bestätigt (gleiche Datei in mehreren Themenordnern, z. B. „Beitrag_HMD…" doppelt hochgeladen) – Fuzzy-Check produziert erwartungsgemäß auch mal einen falschen Alarm (zwei verschiedene EU-Rechtsakte mit sehr ähnlichem CELEX-Dateinamen), das ist als Hinweis zur manuellen Prüfung gedacht, keine automatische Entscheidung, also kein Bug. UI bei 155 Quellen (Tabelle + Mobil-Karten) ohne Konsolenfehler getestet.

---

## Phase 1 abgeschlossen 🎉 (2026-07-25)

Alle zehn Pakete durchlaufen: Ingest, Metadaten-Anreicherung, Ranking-Check, Bibliotheksansicht mit Korrektur, Erfassungsdialog graue Literatur und Dublettenerkennung stehen; 155 echte Quellen (davon 141 aus dem vollständigen realen Bestand) sind erfasst und filterbar. Offene Posten fürs händische Nacharbeiten (kein Blocker für Phase 2): `supabase link`/CLI-DB-Zugriff weiterhin nicht eingerichtet (Migrationen laufen manuell im SQL-Editor), 88 Quellen aktuell `needs_review` (die meisten davon, weil weder DOI noch Metadaten automatisch auffindbar waren – erwartbar bei Fachpresse/deutschsprachigen Quellen ohne Crossref/OpenAlex-Indexierung), CORE-Rankingliste für Konferenzen noch nicht beschafft.
