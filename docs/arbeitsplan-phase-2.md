...
**Notizen:** Backfill selbst war beim Start dieses Pakets bereits vollständig (lief inkrementell über Pakete 2-5 mit) - 150 Quellen mit erfolgreicher Extraktion hatten alle schon Chunks, 0 Chunks ohne Embedding. Der Schwerpunkt lag daher auf dem Qualitätscheck.

10 zufällige Quellen gezogen (gemischt DE/EN, Journal/Buch/undokumentiert, mit/ohne Ranking), je ein Chunk aus der Mitte des Dokuments (vermeidet Verzerrung durch den Überlappungs-Präfix) als Stichprobe genommen. Dabei eine bislang unentdeckte, echte weitere Quelle mit derselben Fehlerklasse wie in Paket 5 gefunden ("Business-IT-Alignment 2017", `a9b67153…`) - Buchstaben-Verschiebungs-Chiffre durch kaputtes CID-Font-Mapping, von den bisherigen Heuristiken nicht erkannt (`extraction_status = extracted`). Diesmal aber, anders als bei den zwei unreparierbaren Fällen aus Paket 5, war nur die Textebene betroffen, nicht das gerenderte Bild selbst - `--force-ocr` hat sauber lesbaren Text geliefert. Quelle neu extrahiert (OCR), gechunkt (85 Chunks) und eingebettet.

Beim anschließenden Live-Test der Korrektur über die Such-UI eine zweite, komplett unabhängig entdeckte Quelle gefunden: `fd100f96…` ("Business-IT-Alignment: Gemeinsam zum Unternehmenserfolg") ist ein **Duplikat derselben Datei** (identischer Original-Dateiname, dieselbe Fehlerklasse) unter anderem Titel - von der Dubletten-Erkennung aus Phase 1 nicht erkannt, weil weder DOI noch Titel-Fuzzy-Match nah genug lagen. Ebenfalls per `--force-ocr` repariert (neu extrahiert, 85 Chunks, eingebettet). **Hinweis an den Nutzer:** `a9b67153…` und `fd100f96…` sind vermutlich dieselbe Quelle zweimal erfasst - bewusst nicht selbst gelöscht/zusammengeführt, das ist eine inhaltliche Entscheidung des Nutzers (Bibliothek zeigt beide weiterhin einzeln an).

Versuch, die Erkennungslücke zu schließen: `needs_ocr()` um ein drittes Signal erweitert (Stoppwort-Anteil DE/EN - eine Buchstaben-Verschiebungs-Chiffre besteht fast nur aus echten Buchstaben, aber "der"/"und"/"the" landen darin so gut wie nie unverändert). Ehrliches Ergebnis beim Test gegen die Originaldaten: **greift nicht zuverlässig**, weil die Korruption bei beiden gefundenen Fällen nur einzelne Wörter/Passagen innerhalb einer ansonsten sauberen Seite betraf (z. B. nur eine Überschrift oder ein Inhaltsverzeichnis-Eintrag) - der Stoppwort-Anteil über die ganze Quelle gemittelt blieb dadurch weit über dem Schwellwert (0,18 statt der erwarteten <0,03). Eine zuverlässige automatische Erkennung bräuchte Analyse auf Absatz- statt Dokumentebene - für dieses Werkzeug als Aufwand nicht gerechtfertigt. Die Erweiterung bleibt als zusätzliche Sicherheitsnetz-Stufe für komplett (nicht nur teilweise) verschlüsselte Dokumente im Code, aber die eigentliche Absicherung bleibt die stichprobenartige menschliche Prüfung - genau das, was dieses Paket gerade vorgeführt hat. Bestandsweite Stoppwort-Analyse über alle 150 erfolgreich extrahierten Quellen lief zusätzlich zur Kontrolle: keine weiteren Ausreißer (niedrigster Wert im ganzen Bestand 0,10, weit über dem Schwellwert).

Seitenangaben-Verifikation: für alle 10 Stichproben den in der DB gespeicherten Seiten-Wert direkt gegen den unabhängig neu extrahierten PDF-Text geprüft (nicht nur der Suchfunktion vertraut). 8 von 10 direkt bestätigt: Stichproben-Text erscheint tatsächlich auf der gemeldeten physischen PDF-Seite. Bei den anderen 2 zunächst scheinbare Abweichungen gefunden, bei genauerer Prüfung aber beide durch das dokumentierte Überlappungs-Design erklärt (Paket 3: die vorangestellte Überlappung eines Chunks stammt bewusst von der Vorseite, der Chunk selbst gehört trotzdem zur Seite seines eigenen ersten Absatzes) bzw. durch einen wiederkehrenden Kolumnentitel, der auf mehreren Seiten gleich lautet - keine echten Bugs, beide durch direkten Blick in den Chunk-Text und die Original-PDF-Seiten aufgeklärt.

PDF-Sprung zusätzlich per Claude in Chrome gegen die echte Produktivumgebung getestet: Suche nach dem reparierten Zitat („Operative Unterstützung") liefert die frisch korrigierte Quelle mit sauber lesbarem Snippet und korrekter Seite.

Bestand nach Abschluss: 148 `extracted`, 2 `ocr_done`, 2 `extraction_failed` (unreparierbare Fälle aus Paket 5, Original-PDF selbst fehlerhaft gerendert), 3 ohne PDF (keine Extraktion nötig) - macht zusammen alle 155 Quellen. 18.910 Chunks, 0 ohne Embedding. **Phase 2 damit abgeschlossen.** 🎉
