...
### Ad-hoc: Button "Verarbeitung fortsetzen" - Ingest-Pipeline teilweise serverseitig (2026-07-30)

Anlass: Rückfrage des Autors, ob der manuelle Worker-Lauf ("in Verarbeitung" hängt fest, bis jemand `littool-worker` lokal aufruft) durch einen Button in der Oberfläche ersetzt werden kann.

**Architektur-Entscheidung (mit dem Autor abgestimmt, s. Chat-Verlauf):** Nicht die ganze Pipeline ist portierbar. `extract-doi` (liest PDF-Metadaten/erste Seiten per PyMuPDF), `extract-fulltext` (PyMuPDF + `ocrmypdf`-Fallback für Scans) und `chunk` (PyMuPDF-Layout-Analyse für Ueberschriften-Heuristik) hängen an einer nativen Python-Bibliothek bzw. einem externen Binary (Ghostscript/Tesseract) - eine Deno-Edge-Function kann das nicht ausführen, und eine Neu-Implementierung der Seiten-genauen PDF-Extraktion in einer zweiten Sprache wäre ein echtes Risiko für die Seiten-Genauigkeit (Belegbarkeit, CLAUDE.md-Grundprinzip). Diese drei Schritte bleiben deshalb lokal per CLI. Alles danach (`enrich-metadata`, `match-ranking`, `detect-duplicates`, `embed`) braucht kein PDF, nur DB- und externe API-Zugriffe (Crossref/OpenAlex/Voyage) - das läuft jetzt server-seitig per Knopfdruck.

**Neue Edge Function `supabase/functions/run-ingest-pipeline/`:** Portiert (gleiches Feldnamen-/Parsing-Prinzip wie die bereits bestehenden `fetch-crossref-metadata`/`openalex-search`-Functions):
- `enrich-metadata` <- `worker/littool_worker/enrich.py` (Primärpfad DOI, Fallback OpenAlex-Titelsuche)
- `match-ranking` <- `worker/littool_worker/ranking.py`. Die CSV-Dateien (`data/rankings/vhb.csv`/`sjr.csv`) sind **nicht** als separate Asset-Dateien eingebunden, sondern als TS-String-Konstanten (`vhb_data.ts`/`sjr_data.ts`) importiert - eine Deploy-Runde mit `rankings/*.csv` als lose Dateien schlug fehl (`--use-api`-Deploy bundelt nur, was tatsächlich importiert wird, kein `Deno.readTextFile` auf nicht mitgelieferte Pfade zur Laufzeit).
- `detect-duplicates` <- `worker/littool_worker/duplicates.py`, inkl. eigener Ratcliff/Obershelp-Ähnlichkeitsfunktion (kein npm-Package nötig) - Ergebnis am echten Bestand geprüft: identische 9 bereits bekannte Dubletten-Flags wie beim letzten lokalen Worker-Lauf.
- `embed` <- `worker/littool_worker/embeddings.py`, aber **nur ein Batch (max. 100 Chunks) pro Aufruf** - das Voyage-Rate-Limit-Pacing (21s zwischen Requests, `MIN_SECONDS_BETWEEN_REQUESTS`) würde bei vielen offenen Chunks das Edge-Function-Timeout reißen. Response meldet die verbleibende Chunk-Anzahl.

**`lib/ingestPipeline.ts` + Bibliothek.tsx:** Der bestehende "N in Verarbeitung"-Hinweisbanner wurde zu einer Karte mit Anleitung (die drei lokalen Befehle als Klartext) und einem Button "▶ Verarbeitung fortsetzen" erweitert. Der Button ruft die Function auf; bei `embed.remaining > 0` ruft das Frontend automatisch erneut auf (mit 21s Pause, gleiches Pacing wie im lokalen Worker), bis nichts mehr offen ist, und lädt danach die Quellenliste neu. Läuft nur, solange die Seite offen bleibt - kein serverseitiger Dauerjob (bewusste Abwägung: ein echter Hintergrund-Job bräuchte pg_cron/pg_net-Infrastruktur nur für einen seltenen Sammel-Klick, ein erneuter Klick später setzt idempotent dort fort, wo `embedding IS NULL` weiterhin zutrifft).

**Ein echter Bug beim Testen gefunden und behoben:** Der erste Klick auf den Button blieb dauerhaft auf "Läuft …" hängen, obwohl ein direkter Aufruf derselben Funktion nur ~2,3s brauchte. Ursache: ein "ist die Komponente noch gemountet"-Ref (`ingestActiveRef`), dessen Cleanup-Funktion in React 19s StrictMode-Dev-Doppelinvoke (mount → cleanup → erneutes mount) einmal synthetisch feuert und den Ref auf `false` setzte, ohne dass ihn danach etwas zurück auf `true` setzte - der `finally`-Block von `handleRunIngestPipeline` prüfte diesen Ref, bevor er `setIngestRunning(false)` aufrief, und tat es deshalb nie. Fix: der Effect-Body selbst setzt den Ref jetzt explizit auf `true` (nicht nur der `useRef`-Startwert), dadurch überlebt er den StrictMode-Doppelinvoke korrekt.
