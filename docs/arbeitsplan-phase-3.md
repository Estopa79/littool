...
**Nachtrag (mobile Prüfung + Nutzer-Feedback zu Deskriptions-/Evaluationsmatrix):** Mobile Darstellung der Evaluationsmatrix verifiziert (Viewport 375×812) - Kriterien-Bereich, Filter und Tabelle stapeln sich sauber, die Matrix bleibt per horizontalem Scroll innerhalb ihres `overflow-x-auto`-Containers bedienbar (Dropdowns pro Zelle erreichbar). Danach auf Wunsch ergänzt:

- **Themenfeld-Filter** in Deskriptions- und Evaluationsmatrix (gleiches Dropdown-Muster wie Bibliothek/Relevanzmatrix, über `fetchAllTopics`/`fetchAllSourceTopics`).
- **Sortierbare Tabellenköpfe** in beiden Matrizen (klickbare `<button>`-Header + Sortpfeil, gleiches Muster wie Bibliothek) - in der Deskriptionsmatrix auch über die fünf Textspalten (alphabetisch), in der Evaluationsmatrix zusätzlich pro Kriteriums-Spalte (nach Zellwert 0-3).
- **CSV-Export** in beiden Matrizen (`Blob` + `URL.createObjectURL`, gleiches Muster wie der bestehende Export in `RelevanceMatrix.tsx`) - exportiert jeweils die aktuell sichtbare (gefilterte/sortierte) Zeilenmenge.
- **Evaluationskriterien-Bereich einklappbar** gemacht (Default zugeklappt, zeigt Kriterienzahl in der Kopfzeile) - gleiches Auf-/Zuklapp-Muster wie der Metadaten-Bereich in `QuellenDetail.tsx`.
- Alle Ergänzungen live im Browser getestet (Filter-Optionen, Sortierung inkl. Kriteriums-Spalte, CSV-Blob-Inhalt per Abfangen von `URL.createObjectURL` geprüft, Auf-/Zuklappen).
